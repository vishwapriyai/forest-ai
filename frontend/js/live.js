...
function droneBannerVisualClass(riskLevel) {
  ...
  return "drone-risk-low";
}

function droneBannerIcon(riskLevel) {
        ...
        <tbody>
          ${filteredRows.map(row => `
            <tr>
              ...
              <td>${row.motion == 1 || row.motion === true || String(row.motion).toLowerCase() === "true" ? "No" : "Yes"}</td>
              <td>${row.smoke}</td>
              <td>${row.sound}</td>
...
function renderDroneRiskBanner(imagePair, payload) {
  ...
  const changePercent = Number(imagePair.change_percent ?? 0);
  const mediumThreshold = Number(payload.drone_thresholds?.medium_change_percent ?? 5);
  const highThreshold = Number(payload.drone_thresholds?.high_change_percent ?? 15);
  const backendRiskLevel = imagePair.risk_level || payload.drone_risk_level || "LOW RISK";
  const riskLevel = backendRiskLevel === "LOW RISK" ? "LOW RISK" : "HIGH RISK";

  let detail = `${changePercent.toFixed(2)}% change detected between yesterday and today for the same grid.`;
  if (backendRiskLevel === "HIGH RISK") {
    detail = `${detail} This crosses the high-risk threshold of ${highThreshold.toFixed(0)}%.`;
  } else if (backendRiskLevel !== "LOW RISK") {
    detail = `${detail} This crosses the warning threshold of ${mediumThreshold.toFixed(0)}%.`;
  } else {
    detail = `${detail} This remains below the warning threshold of ${mediumThreshold.toFixed(0)}%.`;
  }

  banner.className = `drone-risk-banner ${droneBannerVisualClass(riskLevel)}`;
  banner.innerHTML = `
    <div class="drone-risk-main">
      <div class="drone-risk-body">
        <span class="drone-risk-value">${riskLevel}</span>
        <span class="drone-risk-detail">${detail}</span>
      </div>
    </div>
  `;
...
function renderLive(payload) {
  ...
  const isDroneActive = payload.mode && payload.mode.includes("DRONE");

  if (drone) {
    setText("droneGrid", `${drone.grid_id} | ${drone.zone_id}`);
    setText("droneTask", isDroneActive ? `${drone.drone_id} | ${drone.status}` : "Drone offline | sensor mode");
    setText("droneChange", isDroneActive ? `${drone.change_percent.toFixed(2)}%` : "0.00%");
    setText("droneSchedule", formatDate(drone.scheduled_for));
    setText("clusterHealth", payload.sensor_cluster?.health || "-");
  }

  const yImg = document.getElementById("yesterdayImage");
  const tImg = document.getElementById("todayImage");
  const yPlaceholder = document.getElementById("yesterdayPlaceholder");
  const tPlaceholder = document.getElementById("todayPlaceholder");

  if (isDroneActive && imagePair && imagePair.yesterday && imagePair.today) {
    if (yImg.src !== `${API_BASE}${imagePair.yesterday}`) yImg.src = `${API_BASE}${imagePair.yesterday}`;
    if (tImg.src !== `${API_BASE}${imagePair.today}`) tImg.src = `${API_BASE}${imagePair.today}`;
    yPlaceholder.classList.add("hidden");
    ...
    yImg.removeAttribute("src");
    tImg.removeAttribute("src");
    yPlaceholder.classList.remove("hidden");
    tPlaceholder.classList.remove("hidden");
    
    if (isDroneActive) {
      yPlaceholder.innerText = "Drone scanning grid...";
      tPlaceholder.innerText = "Images arriving soon...";
      setText("comparisonLabel", "Drone is currently scanning the grid. Waiting for image processing...");
    } else {
      yPlaceholder.innerText = "Awaiting drone launch...";
      tPlaceholder.innerText = "Awaiting drone launch...";
  ...
  renderDroneRiskBanner(imagePair, payload);
...
document.getElementById("zoneSelector").addEventListener("change", async event => {
  try {
    document.querySelectorAll('.panel').forEach(p => p.classList.add('loading-state'));
...
document.getElementById("launchDrone").addEventListener("click", () => {
  document.querySelectorAll('.panel').forEach(p => p.classList.add('loading-state'));
  toggleDrone(true).catch(console.error);
});
document.getElementById("recallDrone").addEventListener("click", () => {
  document.querySelectorAll('.panel').forEach(p => p.classList.add('loading-state'));
  toggleDrone(false).catch(console.error);
});
